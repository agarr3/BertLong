...
class BertFileModelConfig:
    defaultConfig = BertConfig()
    model_name = 'bert-base-uncased'
    labelEncoderFileName = 'labelEncoder_lbert_file.sav'
    savedModelFileName = 'Bert_File_Model.pt'
    tokenizer = BertTokenizer.from_pretrained(model_name)
    MAX_LEN = 512 #max= 4096
    TRAIN_BATCH_SIZE = 3
    VALID_BATCH_SIZE = 1
    EPOCHS = 3
    LEARNING_RATE = 1e-05
...
class BertFileModel(torch.nn.Module):
    '''
    classdocs
    '''

    def __init__(self, num_classes, configuration=None):
        super(BertFileModel, self).__init__()
        if configuration:
            self.configuration = configuration
        else:
            self.configuration = BertFileModelConfig()
        self.bert_model = BertModel.from_pretrained(self.configuration.model_name)
...
class BertFileClassifier(object):
    '''
    classdocs
    '''
    ...
    def __init__(self, base_dir, configuration=None, mode="eval"):
        ...
        if mode == "eval":
            ...
            self.model = BertFileModel(len(self.lb.classes_), self.configuration)
            PATH = self.modelPath
    ...
    def predict(self, fileName, processed=False):

        self.model.eval()
        if not processed:
            fileName = processFileNamesWithFinanceDictAndClean(fileName)

        if(len(fileName.split(" "))>self.configuration.MAX_LEN):
        ...
        with torch.no_grad():
            ...
            inputs = {
                "input_ids": fileNameIds,
                "attention_mask": fileNameMask
            }
    ...
    def run_training(self, epoch, model, training_data_loader, optimizer):
        ...
        for step, fileBatch in tqdm(enumerate(training_data_loader)):
