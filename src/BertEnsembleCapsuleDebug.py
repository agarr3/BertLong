...
class BertEnsembletModel(torch.nn.Module):
    ...
    def __init__(self, num_classes, configuration=None):
        ...
        flattenedSize = (self.configuration.MAX_LEN + self.configuration.MAX_LEN_FILENAME) * 768
        #self.dense = torch.nn.Linear(flattenedSize, flattenedSize)
        ...
        #self.classifier = torch.nn.Linear(flattenedSize, num_classes)
    ...
    def forward(
        ...
        if exclusion_mask is not None and self.configuration.CONDITIONAL_TRAINING:
            ...
            content_output = content_output[0][:, 0, :]
            filename_output = filename_output[0][:, 0, :]
        ...
        #x = op_bert_ensemble.view(op_bert_ensemble.size(0), -1)
        x = op_bert_ensemble
        x = self.dense(x)
        x = self.dropout(x)
