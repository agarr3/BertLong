...
def translateMood(row):
    if float(row['Avg_Valence']) > 0.0 and float(row['Avg_Arousal']) > 0.0:
        return "Q1"
    elif float(row['Avg_Valence']) < 0.0 and float(row['Avg_Arousal']) > 0.0:
        return "Q2"
    elif float(row['Avg_Valence']) < 0.0 and float(row['Avg_Arousal']) < 0.0:
        return "Q3"
    elif float(row['Avg_Valence']) > 0.0 and float(row['Avg_Arousal']) < 0.0:
...
if __name__ == '__main__':
    runMode = "train"
    BASE_DIR = "bert_mood_capsule_content_mer"
    if(runMode=="train"):
        if not os.path.exists(BASE_DIR):
            os.mkdir(BASE_DIR)

        originalData = pd.read_pickle(os.path.join(BASE_DIR, "MER.pkl"))
